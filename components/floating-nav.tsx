...
export function FloatingNav() {
  ...
  return (
    <>
      ...
      >
        <div className="relative px-4 py-3 rounded-full bg-blue-900/80 backdrop-blur-md border border-blue-700/50 shadow-lg">
          ...
          {isMobile ? (
            <div className="relative flex items-center justify-between">
              <Link href="/" className="font-bold text-lg flex items-center gap-2">
                <img src="/images/samnetlogo1-removebg-preview.png" alt="SAMNET" className="h-5 w-5" />
                <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-300 to-cyan-400">SAMNET</span>
              </Link>
              <Button
                variant="ghost"
                size="icon"
                className="text-blue-300 hover:text-cyan-400 hover:bg-blue-800/50"
            ...
            <div className="relative flex items-center gap-1">
              ...
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className="px-3 py-1 text-sm font-medium text-blue-300 hover:text-cyan-400 transition-colors"
